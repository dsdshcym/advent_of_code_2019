defmodule AoC2019Test do
  ...
  describe "run/1" do
    ...
    test "part 2" do
      assert {_, _, _, [1]} = "3,9,8,9,10,9,4,9,99,-1,8" |> AoC2019.parse([8]) |> AoC2019.run()
      assert {_, _, _, [0]} = "3,9,8,9,10,9,4,9,99,-1,8" |> AoC2019.parse([2]) |> AoC2019.run()

      assert {_, _, _, [1]} = "3,9,7,9,10,9,4,9,99,-1,8" |> AoC2019.parse([7]) |> AoC2019.run()
      assert {_, _, _, [0]} = "3,9,7,9,10,9,4,9,99,-1,8" |> AoC2019.parse([8]) |> AoC2019.run()

      assert {_, _, _, [1]} = "3,3,1108,-1,8,3,4,3,99" |> AoC2019.parse([8]) |> AoC2019.run()
      assert {_, _, _, [0]} = "3,3,1108,-1,8,3,4,3,99" |> AoC2019.parse([9]) |> AoC2019.run()

      assert {_, _, _, [1]} = "3,3,1107,-1,8,3,4,3,99" |> AoC2019.parse([6]) |> AoC2019.run()
      assert {_, _, _, [0]} = "3,3,1107,-1,8,3,4,3,99" |> AoC2019.parse([8]) |> AoC2019.run()
