defmodule AoC2019Test do
  ...
  describe "parse/1" do
    test "returns a values by address map" do
      assert AoC2019.parse("3,225,1,225") == {0, %{0 => 3, 1 => 225, 2 => 1, 3 => 225}, [1], []}
    end
  end

  describe "step/1" do
    test "plus" do
      {:ok, {ip, memory, _, _}} = AoC2019.step(build(:simulator, 0, build(:memory, [1, 2, 3, 0])))
    ...
    test "plus with immediate mode" do
      {:ok, {ip, memory, _, _}} =
        AoC2019.step(build(:simulator, 0, build(:memory, [1101, 30, 23, 0])))
    ...
    test "multiply" do
      {:ok, {ip, memory, _, _}} =
        AoC2019.step(build(:simulator, 1, build(:memory, [0, 2, 2, 3, 0])))
    ...
    test "multiply with immediate mode" do
      {:ok, {ip, memory, _, _}} =
        AoC2019.step(build(:simulator, 0, build(:memory, [1102, 3, 13, 0])))
    ...
    test "input" do
      {:ok, {ip, memory, rest_inputs, _}} =
        AoC2019.step(build(:simulator, 0, build(:memory, [3, 1]), [30, 2]))
    ...
    test "output" do
      {:ok, {ip, memory, _, new_output}} =
        AoC2019.step(build(:simulator, 0, build(:memory, [4, 0]), [], [2]))
  ...
  describe "run/1" do
    test "part 1" do
      {_, _, _, output} = @input |> AoC2019.parse() |> AoC2019.run()
